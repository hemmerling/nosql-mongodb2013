...
homework.c = function() { 
return db.getSisterDB("config").chunks.aggregate( [  { $match : { ns : "week6.trades" } } ,   { $group : { _id : "$shard" , n : { $sum : 1 } } } ] ).result.length;
}
// Homework 6.3
x1 = db.chunks.find( { ns:"week6.trades" }, {min:1,max:1,shard:1,_id:0} ).sort({min:1})
x2 = db.chunks.aggregate( [
